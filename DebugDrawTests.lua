...
function testPhysicsDebug()
    ...
    _:describe("Testing PhysicsDebugDraw", function()
        ...
        _:test("only top card in a stack responds to a BEGIN touch", function()
            ...
            _:expect("--a: touchMap has the right body", debugDraw.touchMap[fakeBeginTouch.id].body).is(card2.body)
            _:expect("--b: right # in stack", #debugDraw.touchMap[fakeBeginTouch.id].stack).is(1)
            _:expect("--c: body is in stack", debugDraw.touchMap[fakeBeginTouch.id].stack).has(card2.body)
        end)
        ...
        -- ok thats next test
        _:test("the stack in a touchMap is a forced stack in the stacker", function()
            setupTwoCardsAndBeginTouch()
            ...
            _:expect("--a: stack is in stacker", stackForCardExists).is(true)
            local isForcedStack = false
            if stackForCardExists then
                isForcedStack = cardTable.stacker.stacks[singleMovingCard].isForcedStack ~= nil
            end
            _:expect("--b: stack is marked isForcedStack", isForcedStack).is(true)
        end)
        ...
        _:test("a card doesn't become a follower if the previous touch was inside its bounds", function()
            ...
            _:expect(#debugDraw.touchMap[fakeBeginTouch.id].stack).is(1) --should only have top card in it
        end)
...
_:test("a card DOES become a follower if the touch started outside its bounds", function()
    ...
    shouldReport = false
    debugDraw:touched(fakeMovingTouch)
    _:expect("--a: two followers if touch started outside card2 then went in", #debugDraw.touchMap[fakeBeginTouch.id].stack).is(2)
    _:expect("--b: card2 is now in stack", debugDraw.touchMap[fakeBeginTouch.id].stack).has(card2.body)
...
_:test("touchMap is cleared when touch is cancelled", function()
    ...
    _:expect("--b: touchMap after BEGAN has one card in stack", #debugDraw.touchMap[fakeBeginTouch.id].stack).is(1)
    debugDraw:touched(fakeMovingTouch)
    _:expect("--c: touchMap still right body after MOVING", debugDraw.touchMap[fakeBeginTouch.id].body).is(card.body)
    _:expect("--d: after MOVING has two followers", #debugDraw.touchMap[fakeBeginTouch.id].stack).is(2)
    debugDraw:touched(fakeEndTouch)
    local touchMapIsNil = debugDraw.touchMap[fakeBeginTouch.id] == nil
...
_:test("cardTable.stacker behavior", function()
    --         if true then return end
    CodeaUnit.doBeforeAndAfter = false
    ...
    debugDraw:touched(fakeBeginTouch)
    debugDraw:touched(fakeMovingTouch)
    local stackerStack = cardTable.stacker.stacks[card]
    _:expect("--b: after touch moves through two bodies, second body is in stacker stack", stackerStack).has(card2)
    ...
    --change touchMap's body to a table of bodies that holds all bodies in a stack...? or not because theres a quicker way that's less elegant but will work...stack to a single table stored in a touchMap's body, so that can directly become a stack...
